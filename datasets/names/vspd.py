...
class Vspd(Dataset):
    def __init__(self, cfg):
        ...
        # filesystem
        self.anno_path = \
            "labels/test_captions.zh.jsonl" if self.mode == "test" else "labels/train_captions.zh.jsonl"
        self.anno_path = path.join(self.cfg.data_path, self.anno_path)
        self.video_path = "shots_48_27p/"
        self.video_path = path.join(self.cfg.data_path, self.video_path)
        
        self.info = load_jsonl(self.anno_path)
        # filter only-1shot
    ...
    def get_shot_info(self, index):
        ...
        # text = item["caption"].strip().split('.')
        # # chinese_text = translate_sentences(text) # 英译中
        # chinese_text = [t for t in text if len(t) > 0]
        return item["caption"], item["shots"]
    
    def get_item(self, index, frame_num=5):
        chinese_text, shot_path_list = self.get_shot_info(index)
        
        # shot:
        shots = []
        for shot in shot_path_list:
            femb = self.clip.encode_video(path.join(self.video_path, shot), frame_num=frame_num)
            shots.append(femb) # L,K
        shots = [s for s in shots if s.shape[0] == frame_num] # 过滤掉过短镜头
        shots_embedding = torch.stack(shots, dim=0) # num_shot, num_frame, emb_size
        
        # text:
        text_embedding = self.clip.encode_text(text_list=chinese_text, return_seq=True) # True:X,K; False:1,K
    ...
    def epoch_stop(self):
        # s, e = self._buffer.get_range()
        # return e >= len(self.info)
    ...
    def __getitem__(self, index):
        ...
        if not self.gen_pos_neg: # 是否构建对应的正负样本对
            return {
...
# def get_vspd_loader(dataset: Vspd, cfg):
#     sampler = DistributedSampler(dataset, shuffle=cfg.shuffle) if cfg.gpu_num > 1 else None
#     return DataLoader(
#         dataset, batch_size=cfg.batch_size, shuffle=cfg.shuffle, sampler=sampler,
